  ...
  <xsl:template match="/">
    <RispostaCertificato xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
      ...
      <generalita>
			    		...
			    		<cognome><xsl:value-of select="//COGN"/></cognome>
					<sesso><xsl:value-of select="//SEX"/></sesso>
					<xsl:if test="//SEX = 'M'">
						<txtNascita><xsl:text>E' NATO</xsl:text></txtNascita>
					</xsl:if>
					  <txtCodiceFiscale>Cod.Fis.</txtCodiceFiscale> 
					<codiceFiscale><xsl:value-of select="//CODFIS"/></codiceFiscale>
					<xsl:if test="//SEX = 'F'">
						<txtNascita>
							<xsl:text>E' NATA</xsl:text>
						</txtNascita>
					</xsl:if>
						<dataNascita><xsl:value-of select="//GNAS"/>/<xsl:value-of select="//MNAS"/>/<xsl:value-of select="//ANAS"/></dataNascita>
						 <txtDataNascita>il</txtDataNascita> 
							<luogoNascita>
							<nomeComune><xsl:value-of select="//COMNAS"/></nomeComune>
							<siglaProvincia><xsl:value-of select="//PRCNAS"/></siglaProvincia>
							  <xsl:if test="//DNAZCNAS = 'ITALIA'">
							     <nomeStato></nomeStato>
							  </xsl:if>
							  <xsl:if test="//DNAZCNAS != 'ITALIA'">
						      <nomeStato><xsl:value-of select="//DNAZCNAS"/></nomeStato>
								</xsl:if>
								 <xsl:if test="//NAZCPRO != 'I'">
						        <siglaStatoProvenienza><xsl:value-of select="//NAZCPRO"/></siglaStatoProvenienza>
								</xsl:if>
						...
						<xsl:if test="//COMPRO != ''">
						<comuneProvenienza><xsl:value-of select="//COMPRO"/></comuneProvenienza>
							<provinciaProvenienza><xsl:value-of select="//PRCPRO"/></provinciaProvenienza>
							  <xsl:if test="//DNAZCPRO = 'ITALIA'">
							     <statoProvenienza></statoProvenienza>
							  </xsl:if>
							  <xsl:if test="//DNAZCPRO != 'ITALIA'">
						      <statoProvenienza><xsl:value-of select="//DNAZCPRO"/></statoProvenienza>
								</xsl:if>
				    	</xsl:if>
				    	<xsl:if test="//COMPRO = ''">	<comuneProvenienza></comuneProvenienza>
						<provinciaProvenienza></provinciaProvenienza>
						 <statoProvenienza></statoProvenienza>
				    	</xsl:if>
				    	</xsl:if>
      ...
      <Variazione>
        <DataVariazione>dal<xsl:value-of select="./GVAR"/>/<xsl:value-of select="./MVAR"/>/<xsl:value-of select="./AVAR"/></DataVariazione>
        <LuogoVariazione> <xsl:value-of select="./EVENTO"/></LuogoVariazione>
       </Variazione>
      </xsl:for-each> 
    </RispostaCertificato>
  </xsl:template>
